...
async function getBonusTracker(userId) {
let tracker = await BonusTracker.findOne({ userId });
if (!tracker) {
tracker = new BonusTracker({ userId });
await tracker.save();
}
return tracker;
}
...
return rewards.consecutives;
}

if ((dice1 % 2 === 0 && dice2 % 2 === 0) || (dice1 % 2 === 1 && dice2 % 2 === 1)) {
return rewards.sameParity;
}
...
password: hashed,
email: process.env.ADMIN_EMAIL || 'admin@lotto.com'
}).save();
...
// Get announcements (personalized)
...
let tracker = await getBonusTracker(user.id);
...
// Check for random roll rewards (non-doubles)
...
// Play Lotto - FIXED: Cannot win same code twice
...
// User already won this code before
bestMatch = -1;
}
...
let message = '';
if (existingWin) {
message = `⚠️ You've already won with code ${code} before! Try a different code. You lost $${stake}.`;
...
if (user.balance === 0) user.balance = 75; // Changed from 500 to 75
await user.save();
...
res.json({ message: `Awarded $${amount} to ${user.username}` });
} catch (error) {
res.status(500).json({ error: error.message });
}
});

app.post('/api/admin/update-credentials', verifyAdmin, async (req, res) => {
try {
...
try {
const { userId, title, message, expiresInDays } = req.body;
...
res.json({ message: targetUser ? `Announcement sent to ${targetUser.username}` : 'Announcement sent to all users' });
} catch (error) {
res.status(500).json({ error: error.message });
}
});
...
try {
const { cryptoWallets, withdrawalFees, bankDetails } = req.body;
let settings = await PaymentSettings.findOne();
...
if (withdrawalFees) {
if (withdrawalFees.standard !== undefined) settings.withdrawalFees.standard = withdrawalFees.standard;
if (withdrawalFees.express !== undefined) settings.withdrawalFees.express = withdrawalFees.express;
if (withdrawalFees.instant !== undefined) settings.withdrawalFees.instant = withdrawalFees.instant;
}
if (bankDetails) {
...
const mostActiveUsers = await Entry.aggregate([
{ $group: { _id: '$userId', count: { $sum: 1 }, username: { $first: '$username' } } },
{ $sort: { count: -1 } },
{ $limit: 5 }
]);

const popularGames = await Entry.aggregate([
...
mostActiveUsers: mostActiveUsers.map(u => ({ username: u.username, count: u.count })),
popularGames: popularGames.map(g => ({ _id: g._id, count: g.count }))
});
} catch (error) {
res.status(500).json({ error: error.message });
}
...
app.post('/api/admin/create-bonus-code', verifyAdmin, async (req, res) => {
try {
res.json({ message: 'Bonus code feature coming soon' });
} catch (error) {
res.status(500).json({ error: error.message });
}
});
...
║ ║
║ ✅ Default Balance: $75 (for new users) ║
║ ✅ Cannot win same code twice ║
║ ✅ Minimum Stake: $5 (no upper limit) ║
║ ✅ Random Roll Rewards Enabled ║
╚══════════════════════════════════════════════════════════╝
`);
});
} catch (error) {
